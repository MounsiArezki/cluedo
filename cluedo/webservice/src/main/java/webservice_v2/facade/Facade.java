...
import webservice_v2.modele.entite.etat_partie.PartieFinie;
...
public class Facade {
    ...
    private Map<String, Partie> listePartiesMongo; //liste de partie sauvegardées pour "simuler mongo"
    ...
    private Facade() {
        ...
        listePartiesMongo = new HashMap<>(); //liste de partie sauvegardées pour "simuler mongo"
    ...
    public Collection<User> getConnectedUsers() { return listeUsersConnectes.values(); }

    // récupérer tous les utilisateurs connectés qui ne sont pas dans une partie
    public Collection<User> getAvailableUsers() {
        Collection<User> pp = new ArrayList<>();
        Collection<User> au = listeUsersConnectes.values();

        for (Partie p : listeParties.values()) // pour chaque partie
            if (!(p.getEtatPartie() instanceof PartieFinie)) // si elle n'est pas terminée
                for (Joueur j : p.getJoueurs().values()) // alors pour chaque joueur
                    pp.add(j.getUser()); // on l'ajoute à la liste

        au.removeAll(pp); // on "soustrait" la liste des joueurs connectés à celle des joueurs en cours de partie
